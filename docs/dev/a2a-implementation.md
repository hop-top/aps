...
**Implementation**:
- Queue directory: `~/.agents/ipc/queues/<profile-id>/incoming/`
...
```bash
# View task metadata
cat ~/.agents/a2a/<profile-id>/tasks/<task-id>/meta.json | jq

# View events
ls -la ~/.agents/a2a/<profile-id>/tasks/<task-id>/event_*.json
```
