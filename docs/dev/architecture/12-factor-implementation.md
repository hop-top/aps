...
### Profile Actions
```yaml
# ~/.agents/profiles/myagent/profile.yaml
actions:
  triage-issue:
    command: |
...
**Files:**
- `~/.agents/profiles/*/profile.yaml` - Profile configuration
...
# APS_PROFILE_PATH=~/.agents/profiles/myagent
...
**Files:**
- `~/.agents/profiles/*/profile.yaml` - One profile = one agent
...
**Files:**
- `~/.agents/profiles/*/profile.yaml` - Stateless configuration
...
| 10. Small Agents | One profile = one agent | `~/.agents/profiles/` | [Profiles](../user/PROFILES.md) |
