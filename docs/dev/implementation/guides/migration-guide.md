...
3. Verify profile isolation level:
   ```bash
   cat ~/.local/share/aps/profiles/profile/profile.yaml | grep "level:"
   ```
...
for profile in $(aps profile list); do
  ...
  # Update profile.yaml
  profile_file=~/.local/share/aps/profiles/$profile/profile.yaml
...
4. **Backup profiles**: Back up your profile directories before migration:
   ```bash
   cp -r ~/.local/share/aps/profiles ~/.local/share/aps/profiles.backup
   ```
