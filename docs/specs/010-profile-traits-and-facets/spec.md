...
- **FR-081**: Perspective-declared derivation functions (FR-061), facet preconditions (FR-037), and gate predicates (FR-032) execute in a sandboxed environment chosen by the implementation plan (per § Open Questions). The sandbox MUST NOT expose baseline trait write APIs to executing code, regardless of the language chosen.
...
- **Derivation-function and predicate language**: derivation functions (FR-061), facet preconditions (FR-037), gate conditions in modulators (FR-032), and Perspective composition overrides (FR-041) all need an executable expression language. Candidates: CEL (Common Expression Language), a small embedded scripting runtime (Lua, Starlark, JS), pure-data JSON-Logic, or compiled WASM. Decision deferred to implementation plan but THE biggest implementation question; whichever language is chosen must be sandboxable, deterministic, and inspectable for the audit trail (FR-062, FR-072).
