...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case "enter":
			...
			m.input.SetValue("")
			if command, ok := ParseMetaCommand(prompt); ok {
				m.auto.Apply(command)
				return m, nil
			}
...
func (m model) View() tea.View {
	...
	if m.streaming {
		...
		b.WriteString("\n")
	} else if m.auto.Mode == corechat.ControlModeAuto {
		b.WriteString(statusStyle.Render(fmt.Sprintf("auto %d/%d", m.auto.AutoTurns, m.auto.MaxAutoTurns)))
		b.WriteString("\n")
	}
