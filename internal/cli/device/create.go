...
var defaultManager = device.NewManager()
...
func runCreate(name, deviceType, strategy, profileID string, jsonOut bool) error {
	dt := device.DeviceType(deviceType)

	if !device.IsDeviceTypeValid(dt) {
		return renderTypeError(deviceType)
	}

	if !device.IsDeviceTypeImplemented(dt) {
		return renderTypeNotImplementedError(dt)
	}

	scope := device.ScopeGlobal
	if profileID != "" {
		scope = device.ScopeProfile
	}

	ls := device.LoadingStrategy(strategy)
	if ls == "" {
		ls = device.DefaultStrategyForType(dt)
	}
...
func renderTypeNotImplementedError(dt device.DeviceType) error {
...
func renderCreateSuccess(dev *device.Device, profileID string) error {
...
func renderCreateJSON(dev *device.Device) error {
