...
func runLink(opts linkOpts) error {
	dev, err := device.LoadDevice(opts.deviceName)
	if err != nil {
		return err
	}
	...
	if dev.Type == device.DeviceTypeMessenger && len(opts.mappings) > 0 {
...
func renderLinkDryRun(dev *device.Device, profileID string, mappings []string) error {
	...
	if dev.Type == device.DeviceTypeMessenger && len(mappings) > 0 {
		fmt.Printf("  Mappings:\n")
...
func renderLinkJSON(dev *device.Device, profileID string, mappings []string) error {
	data := map[string]any{
		"device":   dev.Name,
		"profile":  profileID,
		"linked":   true,
