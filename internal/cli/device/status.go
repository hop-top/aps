...
func renderStatusJSON(dev *device.Device, runtime *device.DeviceRuntime) error {
...
func renderStatusDetail(dev *device.Device, runtime *device.DeviceRuntime, verbose bool) error {
	...
	if dev.Scope == device.ScopeGlobal {
	...
	if runtime.State == device.StateRunning {
		health := runtime.Health
		if health == "" {
			health = "unknown"
		}
	...
	if runtime.State == device.StateFailed {
		fmt.Println()
