...
// AdapterClaims holds the JWT claims for a mobile adapter token
...
// CreateToken generates a JWT token for a mobile adapter
func (tm *TokenManager) CreateToken(device *MobileAdapter, expiry time.Duration) (string, error) {
	if expiry == 0 {
	...
	claims := &AdapterClaims{
		...
		AdapterOS:     device.AdapterOS,
		ProfileID:    tm.profileID,
		Capabilities: device.Capabilities,
		RegisteredClaims: jwt.RegisteredClaims{
			Issuer:    "aps",
			Subject:   device.AdapterID,
...
func (tm *TokenManager) ValidateToken(tokenString string) (*AdapterClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &AdapterClaims{}, func(token *jwt.Token) (any, error) {
	...
	claims, ok := token.Claims.(*AdapterClaims)
	if !ok || !token.Valid {
