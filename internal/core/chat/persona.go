...
func RenderSystemPrompt(p *core.Profile) string {
	...
	if id := strings.TrimSpace(p.ID); id != "" {
		lines = append(lines, "Profile URI: "+p.URI())
	}
	if len(p.Roles) > 0 {
		lines = append(lines, "Roles: "+strings.Join(p.Roles, ", ")+".")
	}
	if lang := strings.TrimSpace(p.Preferences.Language); lang != "" {
		lines = append(lines, "Language: "+lang+".")
	}
	if tz := strings.TrimSpace(p.Preferences.Timezone); tz != "" {
