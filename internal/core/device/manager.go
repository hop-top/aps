...
func (m *Manager) CreateDevice(name string, deviceType DeviceType, strategy LoadingStrategy, scope DeviceScope, profileID string) (*Device, error) {
	if !IsDeviceTypeValid(deviceType) {
		return nil, ErrDeviceTypeInvalid(string(deviceType))
	}

	if !IsDeviceTypeImplemented(deviceType) {
		return nil, ErrDeviceTypeNotImplemented(deviceType)
	}

	exists, err := DeviceExists(name)
	if err != nil {
		return nil, err
	}
	if exists {
		return nil, ErrDeviceAlreadyExists(name)
	}
...
func (m *Manager) StartDevice(ctx context.Context, name string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if runtime, exists := m.runtimes[name]; exists {
		if runtime.State == StateRunning {
			return ErrDeviceAlreadyRunning(name)
		}
	}
...
func (m *Manager) startSubprocess(ctx context.Context, device *Device) error {
	executablePath, err := m.findDeviceExecutable(device)
	if err != nil {
	...
	cmd.Dir = device.Path
	cmd.Env = append(os.Environ(), m.buildDeviceEnv(device)...)
	...
	now := time.Now()
	m.runtimes[device.Name] = &DeviceRuntime{
		Name:      device.Name,
		State:     StateRunning,
...
func (m *Manager) startBuiltin(ctx context.Context, device *Device) error {
	now := time.Now()
	m.runtimes[device.Name] = &DeviceRuntime{
		Name:      device.Name,
		State:     StateRunning,
...
func (m *Manager) StopDevice(ctx context.Context, name string, force bool) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if !exists || runtime.State == StateStopped {
		return ErrDeviceAlreadyStopped(name)
	}
...
func (m *Manager) stopSubprocess(ctx context.Context, device *Device, runtime *DeviceRuntime, force bool) error {
	if runtime.PID == 0 {
		return nil
	}
...
func (m *Manager) stopBuiltin(ctx context.Context, device *Device, runtime *DeviceRuntime) error {
	runtime.State = StateStopped
	return nil
}

func (m *Manager) GetRuntime(name string) (*DeviceRuntime, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *Manager) LinkDevice(deviceName, profileID string) error {
	device, err := LoadDevice(deviceName)
	if err != nil {
		return err
	}
...
func (m *Manager) UnlinkDevice(deviceName, profileID string) error {
	device, err := LoadDevice(deviceName)
	if err != nil {
		return err
	}
...
func (m *Manager) GetDeviceLogs(name string, tail int, follow bool) ([]string, error) {
	device, err := LoadDevice(name)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) Shutdown(ctx context.Context) error {
	...
	for name, runtime := range m.runtimes {
		if runtime.State == StateRunning {
			if err := m.StopDevice(ctx, name, false); err != nil {
...
func (m *Manager) findDeviceExecutable(device *Device) (string, error) {
...
func (m *Manager) buildDeviceEnv(device *Device) []string {
	env := []string{
...
func (m *Manager) ListRunningDevices() []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
