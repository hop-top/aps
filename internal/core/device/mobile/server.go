...
// DeviceServer manages the HTTP + WebSocket server for mobile device connections.
...
// DeviceConnection tracks an active WebSocket connection
...
// NewDeviceServer creates a new device server
func NewDeviceServer(profileID string, registry *Registry, tokenMgr *TokenManager, opts ...ServerOption) *DeviceServer {
	s := &DeviceServer{
		registry:     registry,
		tokenMgr:     tokenMgr,
		profileID:    profileID,
...
type ServerOption func(*DeviceServer)

func WithApprovalRequired(required bool) ServerOption {
	return func(s *DeviceServer) { s.approvalRequired = required }
}

func WithMaxDevices(max int) ServerOption {
	return func(s *DeviceServer) { s.maxDevices = max }
}

func WithTLSCert(cert *tls.Certificate) ServerOption {
	return func(s *DeviceServer) { s.tlsCert = cert }
}

// --- StandaloneProtocolServer interface ---

func (s *DeviceServer) Name() string { return "mobile-device" }

func (s *DeviceServer) Start(ctx context.Context, config any) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *DeviceServer) Stop() error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *DeviceServer) Status() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.status
}

func (s *DeviceServer) GetAddress() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.addr
}
...
func (s *DeviceServer) RegisterPairingCode(code string, capabilities []string, expiry time.Duration) {
	s.pairingMu.Lock()
...
func (s *DeviceServer) ActiveConnections() int {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return len(s.conns)
}
...
func (s *DeviceServer) registerRoutes(mux *http.ServeMux) {
...
func (s *DeviceServer) handleHealth(w http.ResponseWriter, r *http.Request) {
...
func (s *DeviceServer) handlePair(w http.ResponseWriter, r *http.Request) {
	...
	if activeCount >= s.maxDevices {
		writeJSONError(w, fmt.Sprintf("maximum devices reached (%d/%d)", s.maxDevices, s.maxDevices), http.StatusConflict)
		return
	}

	// Generate device ID
	deviceID := generateDeviceID(req.DeviceName, req.DeviceOS)
	...
	// Register device
	if err := s.registry.RegisterDevice(device); err != nil {
...
func (s *DeviceServer) handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	device, err := s.registry.GetDevice(claims.DeviceID)
	if err != nil {
	...
	// Update last seen
	s.registry.UpdateLastSeen(claims.DeviceID)
	...
	for {
		...
		if err := conn.ReadJSON(&msg); err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseNormalClosure) {
				log.Printf("websocket error for %s: %v", claims.DeviceID, err)
			}
			return
		}
...
func (s *DeviceServer) heartbeat(dc *DeviceConnection) {
...
func (s *DeviceServer) handleWSMessage(dc *DeviceConnection, msg *WSMessage) {
	switch msg.Type {
	case "command":
...
func (s *DeviceServer) handleCommand(dc *DeviceConnection, msg *WSMessage) {
...
// --- DeviceConnection methods ---

func (dc *DeviceConnection) WriteJSON(v any) error {
	dc.mu.Lock()
	defer dc.mu.Unlock()
	if dc.closed {
...
func generateDeviceID(name, os string) string {
