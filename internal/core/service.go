...
// service. Email ticket probes are sent from the first literal entry of
// option allowed_senders (when set) so a correctly locked-down service still
// accepts its own probe.
func SyntheticServiceWebhookPayload(service *ServiceConfig) ([]byte, error) {
	if service == nil {
		return nil, fmt.Errorf("service is required")
	}
	payload, err := SyntheticMessageWebhookPayload(service.Adapter)
	if err != nil {
		return nil, err
	}
	if service.Type != "ticket" || strings.ToLower(strings.TrimSpace(service.Adapter)) != "email" {
		return payload, nil
	}
	sender := firstLiteralAllowedSender(service.Options["allowed_senders"])
	if sender == "" {
		return payload, nil
	}
	var body map[string]any
	if err := json.Unmarshal(payload, &body); err != nil {
		return nil, fmt.Errorf("decode synthetic email payload: %w", err)
	}
	body["from"] = "APS Service Test <" + sender + ">"
	out, err := json.Marshal(body)
	if err != nil {
		return nil, fmt.Errorf("encode synthetic email payload: %w", err)
	}
	return out, nil
}

// firstLiteralAllowedSender returns the first allowed_senders entry without
// glob metacharacters, or "".
func firstLiteralAllowedSender(raw string) string {
...
func SyntheticMessageWebhookPayload(adapter string) ([]byte, error) {
	...
	case "email":
		payload, err := json.Marshal(map[string]any{
			"message_id": fmt.Sprintf("<aps-service-test-%d@aps.local>", time.Now().UnixNano()),
			"from":       "APS Service Test <service-test@aps.local>",
			"to":         "inbox@aps.local",
			"subject":    "aps service test",
			"body":       "aps service test",
		...
		return payload, nil
	default:
		return nil, fmt.Errorf("no synthetic webhook payload for adapter %q", adapter)
	}
}
