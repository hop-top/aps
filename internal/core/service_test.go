...
func TestSyntheticServiceWebhookPayload_EmailUsesAllowedSender(t *testing.T) {
	payload, err := SyntheticServiceWebhookPayload(&ServiceConfig{
	...
	payload, err = SyntheticServiceWebhookPayload(&ServiceConfig{ID: "x", Type: "ticket", Adapter: "email", Profile: "inbox"})
	require.NoError(t, err)
	require.NoError(t, json.Unmarshal(payload, &body))
