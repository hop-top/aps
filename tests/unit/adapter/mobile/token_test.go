...
func TestTokenManagerCreation(t *testing.T) {
	...
	t.Run("reloads existing keys", func(t *testing.T) {
		...
		adapter := &mobile.MobileAdapter{
			AdapterID:     "dev-1",
			AdapterName:   "Test Phone",
			AdapterOS:     "iOS",
			Capabilities: []string{"run:stateless"},
		}
		token1, err := tm1.CreateToken(adapter, time.Hour)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		assert.Equal(t, "dev-1", claims.AdapterID)
	})
}
...
func TestCreateToken(t *testing.T) {
	...
	t.Run("creates valid JWT", func(t *testing.T) {
		adapter := &mobile.MobileAdapter{
			AdapterID:     "iphone-test-001",
			AdapterName:   "Test iPhone",
			AdapterOS:     "iOS",
			Capabilities: []string{"run:stateless", "run:streaming"},
		}

		token, err := tm.CreateToken(adapter, 24*time.Hour)
		require.NoError(t, err)
	...
	t.Run("default expiry is 14 days", func(t *testing.T) {
		adapter := &mobile.MobileAdapter{
			AdapterID:   "dev-default",
			AdapterName: "Default Expiry Phone",
			AdapterOS:   "Android",
		}

		token, err := tm.CreateToken(adapter, 0) // 0 = use default
		require.NoError(t, err)
	...
	t.Run("custom expiry", func(t *testing.T) {
		adapter := &mobile.MobileAdapter{
			AdapterID:   "dev-custom",
			AdapterName: "Custom Expiry Phone",
			AdapterOS:   "iOS",
		}

		token, err := tm.CreateToken(adapter, 1*time.Hour)
		require.NoError(t, err)
...
func TestValidateToken(t *testing.T) {
	...
	t.Run("valid token returns correct claims", func(t *testing.T) {
		adapter := &mobile.MobileAdapter{
			AdapterID:     "dev-claims",
			AdapterName:   "Claims Phone",
			AdapterOS:     "Android",
			Capabilities: []string{"run:stateless", "monitor:sessions"},
		}

		token, err := tm.CreateToken(adapter, time.Hour)
		require.NoError(t, err)

		claims, err := tm.ValidateToken(token)
		require.NoError(t, err)

		assert.Equal(t, "dev-claims", claims.AdapterID)
		assert.Equal(t, "Claims Phone", claims.AdapterName)
		assert.Equal(t, "Android", claims.AdapterOS)
	...
	t.Run("token from different key fails", func(t *testing.T) {
		...
		adapter := &mobile.MobileAdapter{AdapterID: "dev-cross"}
		token, err := tm2.CreateToken(adapter, time.Hour)
		require.NoError(t, err)
