...
func setupTestServer(t *testing.T, opts ...mobile.ServerOption) *testServerEnv {
	...
	allOpts := append([]mobile.ServerOption{mobile.WithMaxDevices(10)}, opts...)
	server := mobile.NewDeviceServer("test-profile", registry, tokenMgr, allOpts...)
...
func (e *testServerEnv) pairURL() string {
	return fmt.Sprintf("http://%s/aps/device/test-profile/pair", e.addr)
}

func (e *testServerEnv) healthURL() string {
	return fmt.Sprintf("http://%s/aps/device/test-profile/health", e.addr)
}

func (e *testServerEnv) wsURL() string {
	return fmt.Sprintf("ws://%s/aps/device/test-profile/ws", e.addr)
}
...
func TestServerLifecycle(t *testing.T) {
	...
	t.Run("stops cleanly", func(t *testing.T) {
		...
		server := mobile.NewDeviceServer("p", registry, tokenMgr)
...
func TestPairingEndpoint(t *testing.T) {
	t.Run("valid pairing code returns token", func(t *testing.T) {
		...
		assert.NotEmpty(t, pairResp.DeviceID)
	...
	t.Run("max devices enforced", func(t *testing.T) {
		env := setupTestServer(t, mobile.WithMaxDevices(1))
